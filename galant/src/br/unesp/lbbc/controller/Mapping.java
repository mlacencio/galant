...
public class Mapping {
	...
	public HashMap<String, Double> getTypeFloat(String attName) {
	...
	public HashMap <String, Double> convertIntegerToDouble(String attName){
		...
		for (CyNode node : nodeList) {
			...
			Integer value=0;
			try {
				value = cyNodeAttrs.getIntegerAttribute(node.getIdentifier(),attName);
	...
	public void saveMissingNodes(ArrayList<String> lista){
		for (String elem:lista){System.out.println(elem);}
		JOptionPane.showMessageDialog(null,"saveMissingNodes");
		
	}
	
	public HashMap <String, Double> convertBooleanToDouble(String attName){
		HashMap<String, Double> map = new HashMap<String, Double>();
		Double x = 0.0;
		for (CyNode node : nodeList) {
			Boolean value = cyNodeAttrs.getBooleanAttribute(node.getIdentifier(),attName);
			if (value ==true){x = 1.0;}else{x=0.0;}
	...
	public HashMap<String, double[]> getCompleteHash(String attribute,Boolean isLog) {
		...
		if (type == TYPE_BOOLEAN){att = convertBooleanToDouble(attribute);}
		else if (type == TYPE_FLOATING){att = getTypeFloat(attribute);}
		else if (type == TYPE_INTEGER){att = convertIntegerToDouble(attribute);}
		else{JOptionPane.showMessageDialog(null, "It is not possible to draw attributes of this type");}
